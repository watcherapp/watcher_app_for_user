...
import 'package:watcher_app_for_user/Constants/appColors.dart';
import 'package:watcher_app_for_user/Data/Services.dart';
import 'package:watcher_app_for_user/Modules/MasterAdmin/Components/ComplaintComponent.dart';
...
class _ComplaintCategoryState extends State<ComplaintCategory> {
                            ...
                            ? ListView.separated(
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  return ComplaintComponent(
                                    complaintData: complaintCatList[index],
                                    onremove: () {
                                      _getComplaintCategory();
                                    },
                                  );
  ...
  _addComplaintCategory() async {
    try {
      final internetResult = await InternetAddress.lookup('google.com');
      if (internetResult.isNotEmpty &&
          internetResult[0].rawAddress.isNotEmpty) {
        var body = {"complainName": tempList};
        log(tempList.toString());
        setState(() {
          isSLoading = true;
        });
        Services.responseHandler(
                apiName: "api/admin/addComplainCategory", body: body)
            .then((responseData) {
